<script>
    ...
    let color, icon,
        latestSaleDate, latestSaleValue,
        previousSaleDate, previousSaleValue
    ...
    $: data && (
        ...
        latestSaleDate = dayjs(Object.keys(data[1])[0]).format('ddd, D MMM YYYY'),
        previousSaleValue = Object.values(data[0])[0],
        previousSaleDate = dayjs(Object.keys(data[0])[0]).format('ddd, D MMM YYYY')
    )
